...
package com.amrizal.example.qrcodetester.zxing;

import java.util.Collection;
import java.util.Map;

import com.amrizal.example.qrcodetester.DeviceRegister;
import com.amrizal.example.qrcodetester.IntegerRes;
import com.amrizal.example.qrcodetester.R;
import com.amrizal.example.qrcodetester.StringRes;
...
public final class CaptureActivityHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        ...
        case R.id.return_scan_result:
            ...
            final String contents = intent1.getStringExtra("SCAN_RESULT");
            final String format = intent1.getStringExtra("SCAN_RESULT_FORMAT");
            Log.d(TAG, "contents = " + contents);
            Log.d(TAG, "contents format = " + format);

            switch (reqCode) {
            case IntegerRes.ACTIVITY_CODE_REG_QR:
                DeviceRegister reg = new DeviceRegister(activity);
                reg.setListener(new DeviceRegister.RegisterCallback() {
                    ...
                    public void OnRegisterResult(boolean isSuccess) {
                        ...
                        if (isSuccess) {
                            ...
                            Log.d(TAG, "pass device register");
                            activity.setResult(Activity.RESULT_OK, intent1);
                            activity.finish();
                        } else {
                            activity.restartPreviewAfterDelay(0);
                        }
                    }
                });

                reg.execute(contents);
                break;

            /*case StringRes.ACTIVITY_CODE_TRANS_QR:
                Log.d(TAG, "not device register mode, returning result....");
                activity.setResult(Activity.RESULT_OK, intent1);
                activity.finish();
                break;*/
            }
    ...
    public void quitSynchronously() {
        Log.d(TAG, "running quitSynchronously()...");
        state = State.DONE;
